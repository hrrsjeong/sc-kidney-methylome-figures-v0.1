...
qsub_list = f"wgbstools_combine_sample_pat_{rannum}.sh"
...
sample_pat_out = "sample_pat"
sample_beta_out = "sample_beta"
os.system(f"mkdir -p {sample_pat_out}")
os.system(f"mkdir -p {sample_beta_out}")

flist =glob.glob("bam2pat/*__*.sorted.pat.gz")
samples = list(set([x.split('/')[-1].split('__')[0].split('.')[0] for x in flist]))

fout = open(qsub_list,'w')
for sample in samples:
    flist =glob.glob(f"bam2pat/{sample}__*.sorted.pat.gz")
    pat_out = f"{sample_pat_out}/{sample}"
    beta_out = f"{sample_beta_out}/{sample}"
    command = f' && ~/Programs/wgbs_tools/wgbstools merge --genome hg38 -f -p {pat_out} {" ".join(flist)}'
    command += f" && ~/Programs/wgbs_tools/wgbstools pat2beta -f --threads {threads} --genome hg38 -o {sample_beta_out} {pat_out}.pat.gz"
    run_systemstr = 'sbatch --wrap=' + systemstr + command
    run_systemstr += '" '+sbatch_option+f' -e {cwd}/err_combine_pat__{sample}.log -o {cwd}/out_combine_pat__{sample}.log'
