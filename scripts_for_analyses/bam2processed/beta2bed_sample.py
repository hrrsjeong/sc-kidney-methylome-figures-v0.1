...
flist = glob.glob("./sample_major_beta/SDKZ*__*.beta")
out_dir = "sample_major_beta2bed"#beta2bed_altered"
os.system(f"mkdir -p {out_dir}")

for f in flist:
    sample = f.split("/")[-1].split(".")[0]
    cmd = ' echo ""'
    #cmd += " && wgbstools beta2bed --genome hg38 "+f+f" >| {out_dir}/"+sample+".bed"
    cmd += f" && python check_cov.py {out_dir}/{sample}.bed {out_dir}_cov/{sample}.cov"
    #exe_list.append(cmd)

#celltype_beta_altered

os.system("mkdir -p cell_class_beta2bed/")
flist = glob.glob("cell_class_beta/*.beta")
for f in flist:
    sample = f.split("/")[-1].split(".")[0]
    if os.path.exists("cell_class_beta2bed/"+sample+".bed"):
        continue
    cmd = ' echo ""'
    cmd += " && ~/Programs/wgbs_tools/wgbstools beta2bed --genome hg38 "+f+" >| cell_class_beta2bed/"+sample+".bed"
    exe_list.append(cmd)
