...
fout = open("sample_covariate.txt", "w")
fout.write("Sample\tSample_id\tCelltype\tState\tAge\tGender\tDisease_level\tPct_cortex\tAge_level\n")
flist = glob.glob("./DSS_input/rds/*.rds")
for sfile in flist:
    ...
    if sample_id in dd:
        ...
        fout.write(sname + "\t" + sample_id+'\t'+ celltype +'\t'+"\t".join(dd[sample_id]) + "\n")
fout.close() 
